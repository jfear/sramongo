...
def parse_sra_experiment(root):
    ...
    sra.library_strategy = get_xml_text(
        root, "EXPERIMENT/DESIGN/LIBRARY_DESCRIPTOR/LIBRARY_STRATEGY"
    )
    sra.library_source = get_xml_text(root, "EXPERIMENT/DESIGN/LIBRARY_DESCRIPTOR/LIBRARY_SOURCE")
    sra.library_selection = get_xml_text(
        root, "EXPERIMENT/DESIGN/LIBRARY_DESCRIPTOR/LIBRARY_SELECTION"
    )
    sra.library_construction_protocol = get_xml_text(
        root, "EXPERIMENT/DESIGN/LIBRARY_DESCRIPTOR/LIBRARY_CONSTRUCTION_PROTOCOL"
    )
    ...
    sra.sra_update_date = datetime.utcnow()
    return sra
...
def add_library_layout(root, sra):
    if root.find("EXPERIMENT/DESIGN/LIBRARY_DESCRIPTOR/LIBRARY_LAYOUT/SINGLE") is not None:
        ...
        sra.library_layout_length = make_number(
            get_xml_attribute(
                root, "EXPERIMENT/DESIGN/LIBRARY_DESCRIPTOR/LIBRARY_LAYOUT/PAIRED", "NOMINAL_LENGTH"
            ),
            int,
        )
        ...
        sra.library_layout_sdev = make_number(
            get_xml_attribute(
                root, "EXPERIMENT/DESIGN/LIBRARY_DESCRIPTOR/LIBRARY_LAYOUT/PAIRED", "NOMINAL_SDEV"
            ),
            float,
        )
...
def add_platform_information(root, sra):
    ...
    sra.instrument_model = get_xml_text(
        root, f"EXPERIMENT/PLATFORM/{sra.platform}/INSTRUMENT_MODEL"
    )
    return sra
